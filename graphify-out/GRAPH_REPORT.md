...
## Corpus Check
- 84 files · ~1,271,586 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1282 nodes · 3323 edges · 34 communities detected
- Extraction: 72% EXTRACTED · 28% INFERRED · 0% AMBIGUOUS · INFERRED: 943 edges (avg confidence: 0.78)
...
1. `_handle_postback()` - 126 edges
2. `build_postback_data()` - 50 edges
3. `build_postback_data()` - 45 edges
4. `handle_webhook()` - 44 edges
5. `ParsedPostback` - 44 edges
6. `_as_dict()` - 39 edges
...
Cohesion: 0.05
Nodes (138): _body_text(), _bool_th(), build_batch_complete_card(), build_confirmation_card(), build_duplicate_warning_card(), build_help_menu_message(), build_history_batch_list_message(), build_history_detail_message() (+130 more)

### Community 1 - "Community 1"
Cohesion: 0.04
Nodes (119): build_history_detail_message(), build_history_summary_message(), _history_detail_date_range(), _history_detail_footer_meta(), _history_detail_header(), _history_detail_meter_ids(), _history_detail_meter_row(), _history_detail_progress_panel() (+111 more)

### Community 2 - "Community 2"
Cohesion: 0.06
Nodes (87): ParsedPostback, _handle_postback(), Protocol, BatchProgress, clear_collection_meter(), clear_collection_skip_meters(), clear_pending_confirmation(), clear_pending_report_import() (+79 more)

### Community 3 - "Community 3"
Cohesion: 0.07
Nodes (74): ParsedCommand, _after_successful_confirmation(), _build_duplicate_warning_for_pending(), _build_reply(), _build_settings_edit_prompt(), _build_settings_input_reply(), _build_status_card_message(), _build_status_message() (+66 more)

### Community 4 - "Community 4"
Cohesion: 0.04
Nodes (56): calculate_reading(), _calculate_replacement_reading(), _decimal_from_row(), _existing_reading_in_batch(), format_reading_value(), _get_default_rate_setting(), _get_last_value(), _get_rate() (+48 more)
...
Cohesion: 0.07
Nodes (64): build_help_flow_response(), _is_https_url(), _line_card_asset_url(), _start_collection_hero_url(), build_meter_request_message(), build_help_flow_response(), build_help_menu_message(), _help_action_tile() (+56 more)

### Community 7 - "Community 7"
Cohesion: 0.06
Nodes (40): build_confirmation_message(), cancel_pending(), clear_pending_confirmation(), confirm_pending(), _confirmation_method(), create_pending_confirmation(), _create_pending_confirmation_record(), _decimal_or_none() (+32 more)
...
Cohesion: 0.08
Nodes (28): build_progress_message(), _expected_meter_count(), format_progress_message(), generate_batch_id(), get_batch_progress(), get_or_create_batch(), _iso_week(), _now() (+20 more)
...
Cohesion: 0.25
Nodes (24): append_audit_log(), append_batch(), append_pending_confirmation(), append_reading(), _enqueue_if_sqlite(), get_active_meters(), get_batch_by_id(), get_batches_by_source() (+16 more)
...
## Knowledge Gaps
- **78 isolated node(s):** `SyncState`, `Download image from LINE Content API and save to disk.      Returns the local fi`, `Send a text message via LINE push API (no reply token needed).`, `Send one or more prepared LINE message objects via push API.`, `Send an image message via LINE push API (no reply token needed).` (+73 more)
...
- **Why does `_handle_postback()` connect `Community 2` to `Community 0`, `Community 1`, `Community 3`, `Community 6`, `Community 9`, `Community 10`, `Community 11`?**
  _High betweenness centrality (0.223) - this node is a cross-community bridge._
- **Why does `_process_ocr_and_confirm()` connect `Community 3` to `Community 0`, `Community 1`, `Community 2`, `Community 4`, `Community 7`, `Community 8`, `Community 17`?**
  _High betweenness centrality (0.191) - this node is a cross-community bridge._
- **Why does `parse_meter_value()` connect `Community 8` to `Community 3`, `Community 5`?**
  _High betweenness centrality (0.116) - this node is a cross-community bridge._
- **Are the 109 inferred relationships involving `_handle_postback()` (e.g. with `set_collection_state()` and `clear_collection_skip_meters()`) actually correct?**
  _`_handle_postback()` has 109 INFERRED edges - model-reasoned connections that need verification._
- **Are the 42 inferred relationships involving `build_postback_data()` (e.g. with `build_report_unavailable_message()` and `build_report_import_prompt_message()`) actually correct?**
  _`build_postback_data()` has 42 INFERRED edges - model-reasoned connections that need verification._
- **Are the 28 inferred relationships involving `handle_webhook()` (e.g. with `parse_postback_action()` and `get_collection_state()`) actually correct?**
  _`handle_webhook()` has 28 INFERRED edges - model-reasoned connections that need verification._